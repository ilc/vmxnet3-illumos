		...
		int min, int max, int def)
{
	...
	if (ddi_prop_lookup_int_array(DDI_DEV_T_ANY, dp->dip, DDI_PROP_DONTPASS,
		...
		if (dp->instance < nprops) {
			...
			vmxnet3_warn(dp, "property %s not available for"
			    "this device\n", name);
		}
		ddi_prop_free(props);
	}
...
void
vmxnet3_warn(vmxnet3_softc_t *dip, char *fmt, ...)
{
	va_list	ap;
	char	buf[256];
	...
	cmn_err(CE_WARN, "%s:%d: %s", VMXNET3_MODNAME, dip->instance, buf);
}

void
vmxnet3_debug(vmxnet3_softc_t *dip, int level, char *fmt, ...)
{
	va_list	ap;
	char	buf[256];
	...
	if (level <= VMXNET3_DEBUG_LEVEL) {
		...
		cmn_err(CE_CONT, "%s:%d: %s", VMXNET3_MODNAME,
		    dip->instance, buf);
	}
}
